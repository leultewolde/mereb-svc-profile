#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
ROVER_BIN="$(${ROOT_DIR}/scripts/graphos/install-rover.sh)"

APOLLO_KEY="${ROVER_APOLLO_KEY:-${APOLLO_KEY:-}}"
if [[ -z "${APOLLO_KEY}" ]]; then
  echo "ROVER_APOLLO_KEY or APOLLO_KEY must be set" >&2
  exit 1
fi

GRAPHOS_GRAPH_ID="${GRAPHOS_GRAPH_ID:-mereb-supergraph}"
GRAPHOS_VARIANT="${GRAPHOS_VARIANT:-${DEPLOY_ENV:-dev}}"
SUBGRAPH_NAME="${SUBGRAPH_NAME:-}"

if [[ -z "${SUBGRAPH_NAME}" ]]; then
  echo "SUBGRAPH_NAME must be set" >&2
  exit 1
fi

SERVICE_NAME="${SERVICE_NAME:-svc-${SUBGRAPH_NAME}}"
SCHEMA_PATH="${SCHEMA_PATH:-${ROOT_DIR}/schema.graphql}"

if [[ ! -f "${SCHEMA_PATH}" ]]; then
  echo "Schema file not found: ${SCHEMA_PATH}" >&2
  exit 1
fi

if [[ "${GRAPHOS_VARIANT}" == "prd" ]]; then
  ROUTING_URL="http://${SERVICE_NAME}.apps-prd.svc.cluster.local/graphql"
else
  ROUTING_URL="http://${SERVICE_NAME}-${GRAPHOS_VARIANT}.apps-${GRAPHOS_VARIANT}.svc.cluster.local/graphql"
fi

echo "Publishing ${SUBGRAPH_NAME} to ${GRAPHOS_GRAPH_ID}@${GRAPHOS_VARIANT}"
APOLLO_KEY="${APOLLO_KEY}" "${ROVER_BIN}" subgraph publish "${GRAPHOS_GRAPH_ID}@${GRAPHOS_VARIANT}" \
  --name "${SUBGRAPH_NAME}" \
  --schema "${SCHEMA_PATH}" \
  --routing-url "${ROUTING_URL}"
